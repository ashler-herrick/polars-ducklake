"""Declarative workload pairs for the bench harness.

Each workload knows how to produce two equivalent reads against a
:class:`bench.fixtures.BenchLake`:

* a polars-ducklake :class:`polars.LazyFrame` (consumed via ``.collect()``)
...
WORKLOADS: tuple[Workload, ...] = (
    Workload("tiny_limit", _wl_tiny_limit, _sql_tiny_limit),
    Workload("full_scan", _wl_full_scan, _sql_full_scan),
    Workload("predicate_partition", _wl_predicate_partition, _sql_predicate_partition),
    Workload(
        "predicate_nonpartition",
        _wl_predicate_nonpartition,
        _sql_predicate_nonpartition,
    ),
    Workload("time_travel", _wl_time_travel, _sql_time_travel),
    Workload("schema_evolved", _wl_schema_evolved, _sql_schema_evolved),
)
